...
Uses the previous day's closing price as the prediction for the next day. This provides a simple benchmark to determine whether the machine learning and statistical models provide meaningful improvement.
...
A time-series forecasting model developed by Facebook (Meta). Unlike the saved LSTM model, Prophet refits on the data selected by the user each time a forecast is requested. Therefore, it can be used with different stock tickers and does not depend on a fixed historical price range.
