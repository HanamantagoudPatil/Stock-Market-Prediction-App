...
# ----------------------------------------------------------------------
# Load production artifacts (cached so they load once per session, not per click)
# ----------------------------------------------------------------------
@st.cache_resource
def load_artifacts():
...
# ----------------------------------------------------------------------
# LSTM forecast — REAL model, not the old np.random.uniform() placeholder
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Prophet forecast — refit LIVE on whatever data the user selects.
# This is the fix for the earlier bug: the old app used a model that was only
# ever trained through 2023, so "future" predictions were a stale 2-year
# extrapolation. Refitting live means it's never stale, and it also works for
# any ticker (unlike the LSTM, which is only valid for the ticker it was trained on).
# ----------------------------------------------------------------------
...
with st.sidebar:
    ...
    end_date = st.date_input("End Date", value=pd.to_datetime("2026-08-02"))
...
if df.empty:
    st.error("No data returned. Check the ticker symbol or widen the date range.")
    st.stop()
